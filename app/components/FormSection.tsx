...
const Input = ({name, items, setItems}: {name:string,  items: string[], setItems: React.Dispatch<React.SetStateAction<string[]>>}) => {
    ...
    const [visibleItems, setVisibleItems] = useState(36); // Show 12 genres initially

    // Show More/Show All button handler
    const handleShowMore = () => {
        setVisibleItems(Object.keys(items).length); // Show all genres
    };
        ...
        <div className="flex min-h-screen">
            ...
            >
                ...
                <h1 className="text-2xl rounded-lg bg-white p-2 font-bold text-gray-800 text-center mb-4">
                    Select the {name}s you&apos;d like to cook with
                </h1>
